...
{
	if ( 'object' !== typeof oValues ||
		! oValues.hasOwnProperty( 'K' ) ||
		! oValues.hasOwnProperty( 'D' ) ||
		! oValues.hasOwnProperty( 'J' ) ||
		! oValues.hasOwnProperty( 'Ks' ) ||
		! oValues.hasOwnProperty( 'Ds' ) ||
		! Array.isArray( oValues.K ) ||
		! Array.isArray( oValues.D ) ||
		! Array.isArray( oValues.J ) ||
		! Array.isArray( oValues.Ks ) ||
		! Array.isArray( oValues.Ds ) )
	{
		return console.log( `failed to calculate kdj indicator.` );
	}

	let arrNew	= [];
	for ( let i = 0; i < oValues.K.length; i ++ )
	{
		arrNew.push({
			close	: arrClose[ i ],
			low	: arrLow[ i ],
